...
def generate_icebreakers(
    ...
    prompts: list[str] = []
    activity = vocab.ACTIVITIES.get(str(user_a.get("match_type")), {}).get(
        str(user_a.get("activity")), "这项活动"
    )
    prompts.append(f"这周做“{activity}”时，你最希望先完成哪一个小目标？")

    common_interests = sorted(set(user_a.get("interests", [])) & set(user_b.get("interests", [])))
    if common_interests:
        interest = vocab.INTEREST_TAGS.get(common_interests[0], common_interests[0])
