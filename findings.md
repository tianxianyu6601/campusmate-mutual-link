...
## 2026-08-09 周期问卷开放选项专项

- 用户线上截图确认：问卷页“更换搭子类型”未返回周期匹配页，而是仍停留在当前问卷。
- 固定地点列表不能覆盖海淀公园等真实地点；固定学习内容列表不能覆盖物理等学科。地点、具体活动/学习内容等开放世界字段必须支持用户直接键入自定义值。
- 性别偏好、是否接受跨校、时间重叠下限等决定安全性与算法语义的字段仍需受控枚举并在服务端校验，不能一律开放为任意文本。
- 本轮需要用反例证明：硬条件不满足绝不匹配；低于最低分不强配；同一用户每轮最多一组；相同输入结果稳定；自定义地点/内容能进入快照和共同点计算。
- 根因定位：`pages/questionnaire.py` 的“更换搭子类型”按钮直接 `st.switch_page("pages/home.py")`，而类型选择器实际位于 `pages/cycle_match.py`。
- 问卷元数据中 `activity`、`acceptable_locations`、`interests` 都是封闭枚举；`data.schema` 又拒绝任何未知值，因此仅改前端下拉框不足以支持自定义。
- 当前 Streamlit 版本的 `st.selectbox` 与 `st.multiselect` 均原生支持 `accept_new_options=True`，无需自制控件。
- 个人资料页的“常用地点”和“自定义兴趣”已经支持自由输入；本次应保持该能力，并让行动卡采用同一产品语义。
- 当前算法已具备活动完全一致、连续共同时间、共同地点、双向水平接受、人数偏好和明确限制等硬过滤；服务层另设 45 分最低互惠分阈值、历史未匹配补偿和重复搭子惩罚。需要补强自定义值规范化、确定性和更细的未匹配原因测试。
- 本地浏览器已验证自定义“量子物理讨论”和“圆明园东门”可录入；“更换搭子类型”一次点击回到周期匹配类型选择器。
- 服务端报名原先仅检查行动卡是字典且含 `user_id`，未重新执行完整 Schema 校验，也未确认行动卡匿名编号属于当前账号；这是异常数据进入匹配池的真实风险，需在报名和最终匹配两处复核。
- 更新后的确定性模拟：50/100/200人数据分别形成20/36/72对，匹配成员均无重复，入选配对最低真实兼容分分别为69.0/67.4/66.7；未匹配者保留是因为没有满足硬条件或全局组合限制，而不是算法崩溃或强行乱配。

## 2026-08-09 新一轮问题定位

- 用户要求联系方式属于“本次活动/本次申请”的业务数据：发起人在发布时必填并直接展示；申请人在申请时必填并直接展示给发起人，不能再要求对方打开个人资料猜测可见性。
- 线上一次点击登录可完成，但从提交到首页稳定出现约 6 秒，缺少即时反馈使用户容易再次点击。
- 登录成功路径同时在登录页和认证外壳写 Cookie，并包含 0.15 秒人工暂停；退出路径包含回调 rerun、同步撤销会话、Cookie 删除、0.15 秒暂停和再次 rerun。
- 认证外壳在页面内容运行前保存变化后的最后路由和会话状态，数据库往返会延后页面绘制并放大旧页面淡化/残影。
- 优化方向：Cookie 只写一次；登录提交后由认证外壳完成持久化；退出同轮完成且跳过恢复；会话保存移到页面内容渲染后。
- 本地回归发现：未登录导航只注册登录页后，登录页不能在同一轮直接 `st.switch_page("pages/home.py")`；正确做法是成功后自动 `st.rerun()`，让入口先按认证状态注册页面，再由入口恢复到 `/home`。这解释了部分“第一次提交无跳转”的历史表现。
...
- 原周期匹配页面是“当前用户即时从CSV演示数据和注册资料中挑选候选人”，没有统一报名池、轮次快照或持久化结果，不符合周期匹配语义。
- 规范化个人资料保存在 `profiles`，算法行动卡保存在 `user_profiles`；报名快照必须同时冻结两者，否则服务层资料完整但算法没有可用输入。
